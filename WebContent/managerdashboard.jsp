            ...
            <div class="col-md-10">
                <div class="dashboard">
                    ...
                    if(que.equals("1"))
                    {
                    	...
                    	out.write("<thead><tr><th scope='col'>Leave Id</th><th scope='col'>leader id</th><th scope='col'>Employee Name</th><th scope='col'>FROM</th><th scope='col'>TO</th><th scope='col'>Reason</th><th scope='col'>APPROVE?</th></thead><tbody>");
                    }
                    int myid=(int)session.getAttribute("userid");
                    String sql="SELECT e.firstname AS 'ename',l.leader,l.lid AS 'lid',sdate,edate,reason,l.`action`  FROM employee e INNER JOIN leaves l  WHERE e.id=l.empid AND l.leader='"+myid+"' and l.`action`='Leader Accepted'";
                    ...
                    %>
                                    <tr>
					      			...
					      			<td><%=rs.getInt("l.leader") %></td>
					      			<td><%=rs.getString("ename") %></td>
					      			<td><%=rs.getDate("sdate") %></td>
					     			<td><%=rs.getDate("edate") %></td>
					     			<td><%=rs.getString("reason") %></td>
					     			<td><a href="managerleaveaccept.jsp?id=<%=rs.getInt("lid")%>">Approve</a>
					     			<a href="managerleavereject.jsp?id=<%=rs.getInt("lid")%>">Reject</a></td>
					     			</tr>              
                     <%}
